...
class TCN(nn.Module):
    def __init__(self, 
        ...
        self.head = Regressor(inc_dim=num_channels[-1], out_dim=out_dim, dims_list=head_dims, dropout=head_dropout, act=nn.ReLU(), has_tanh=False)
    ...
    def forward(self, x):
        ...
        x = x.permute(1, 2, 0)   # input should have dimension (N, C, L)
        y1 = self.tcn(x)
        y1 = y1.permute(2, 0, 1)
        o = self.head(y1)
        # o = self.linear(y1)
...
if __name__ == '__main__':
    tcn = TCN(512, 12, [512], 32, dropout=0.3, head_dropout=0.1, head_dims=[512, 256], seq_len=128)
    input = torch.randn(128,32, 512)
    out = tcn(input)
    print(out.shape)